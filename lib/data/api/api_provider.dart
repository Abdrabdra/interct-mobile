...
class ApiProvider {
  ...
  Future<SingleAvtoAdvModel?> getSingleAvtoAdvers(int id) async {
    ...
    if (tokenBox.get('access') != null) {
      ...
      if (response.statusCode == 200) {
        ...
        SingleAvtoAdvModel saa = SingleAvtoAdvModel(
          id: id,
          price: price,
          cityFrom: cityFrom,
          cityTo: cityTo,
  ...
  Future<bool> likeAdver(int id) async {
    ...
    // print('REGISTER LIKE ADVERS STATUS CODE IS ${response.statusCode}');
  ...
  Future<UserMeModel?> getUserMe() async {
    ...
    if (tokenBox.get('access') != null) {
      ...
      if (response.statusCode == 200) {
        ...
        String phone = markasData['phone'];
        String name = markasData['username'] ?? 'Неизвестно';

        UserMeModel umm = UserMeModel(id: id, name: name, phone: phone);
        return umm;
      }
    }
    return null;
